...
- Writes artifacts under `/tmp/calais-binance-testnet` by default, or a review bundle such as `reports/evidence/testnet` when `--output-dir` is provided.
...
| Final PDF report | COVERED | `reports/latex/report.pdf` is generated from the LaTeX source. |
| AI usage disclosure | COVERED | `AI_USAGE.md`. |
| Clear commit history | COVERED after this task commit | Task 12 changes are committed intentionally for final external submission. |
