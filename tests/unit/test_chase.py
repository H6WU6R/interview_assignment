...
def test_adverse_only_sell_waits_when_desired_moves_up() -> None:
    assert (
        should_reprice(
            Side.SELL,
            active_order_price=Decimal("100"),
...
def test_non_positive_active_order_price_returns_zero_diff() -> None:
    assert reprice_difference_bps(Decimal("100"), Decimal("0")) == Decimal("0")
    assert reprice_difference_bps(Decimal("100"), Decimal("-100")) == Decimal("0")


def test_non_positive_active_order_price_waits_without_division_crash() -> None:
    assert (
        should_reprice(
            Side.BUY,
