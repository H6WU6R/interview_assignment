...
def test_execution_running_can_move_to_cancelling() -> None:
    assert (
        transition_execution(ExecutionStatus.RUNNING, ExecutionStatus.CANCELLING)
        is ExecutionStatus.CANCELLING
    )


def test_child_pending_cancel_can_fill_or_cancel() -> None:
    assert (
        transition_child(ChildOrderStatus.PENDING_CANCEL, ChildOrderStatus.FILLED)
        is ChildOrderStatus.FILLED
    )
    assert (
        transition_child(ChildOrderStatus.PENDING_CANCEL, ChildOrderStatus.CANCELLED)
        is ChildOrderStatus.CANCELLED
    )


def test_child_pending_cancel_can_reconcile_to_still_live_order() -> None:
    assert (
        transition_child(ChildOrderStatus.PENDING_CANCEL, ChildOrderStatus.OPEN)
        is ChildOrderStatus.OPEN
    )
    assert (
        transition_child(
            ChildOrderStatus.PENDING_CANCEL, ChildOrderStatus.PARTIALLY_FILLED
        )
        is ChildOrderStatus.PARTIALLY_FILLED
    )
...
def test_child_unknown_can_be_reconciled_to_exchange_truth() -> None:
    assert (
        transition_child(ChildOrderStatus.UNKNOWN, ChildOrderStatus.OPEN)
        is ChildOrderStatus.OPEN
    )
    assert (
        transition_child(ChildOrderStatus.UNKNOWN, ChildOrderStatus.PARTIALLY_FILLED)
        is ChildOrderStatus.PARTIALLY_FILLED
    )
    assert (
        transition_child(ChildOrderStatus.UNKNOWN, ChildOrderStatus.FILLED)
        is ChildOrderStatus.FILLED
    )
    assert (
        transition_child(ChildOrderStatus.UNKNOWN, ChildOrderStatus.CANCELLED)
        is ChildOrderStatus.CANCELLED
    )
    assert (
        transition_child(ChildOrderStatus.UNKNOWN, ChildOrderStatus.REJECTED)
        is ChildOrderStatus.REJECTED
    )
